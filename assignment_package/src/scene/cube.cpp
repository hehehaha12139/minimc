...
void Cube::create()
{
    ...
    glm::vec4 additionalInfo[CUB_VERT_COUNT];
    for(int i = 0; i < CUB_VERT_COUNT; ++i)
    {
        additionalInfo[i] = glm::vec4(0, 0, 0, 0);
    }
    ...
    glm::vec4 posNorCol[CUB_VERT_COUNT * 4];
    for(int i = 0; i < CUB_VERT_COUNT; i++){
        posNorCol[i * 3] = sph_vert_pos[i];
        posNorCol[i * 3 + 1] = sph_vert_nor[i];
        posNorCol[i * 3 + 2] = cub_vert_col[i];
        posNorCol[i * 3 + 3] = additionalInfo[i];
    }
    ...
    context->glBufferData(GL_ARRAY_BUFFER, CUB_VERT_COUNT * sizeof(glm::vec4) * 4, posNorCol, GL_STATIC_DRAW);
}
